...
namespace EmbeddedCCSCheck
{
    ...
    {
        static readonly string header = "Movie,ZMW,Ref,Pos,Length,Type,AtEnd,RefBP,AltBP,IndelType,InHP,Bases,HPLen,HPChar,Read,llseq,llmut";

        public static string GetHeader() {
            return header;
        }

        public static void OutputVariants(Variant v, string movieName, string holeNumber, string[] readName, double[] LL, double[] mutLL, StreamWriter sw) {
            ...
            lock (sw) {
                for (int i = 0; i < LL.Length; i++) {
                    sw.Write (common);
                    sw.Write (readName[i]+ "," + LL [i] + ", " + mutLL [i] + "\n");
                }
            }
        }
    }
