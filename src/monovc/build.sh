...
# the C++ bits
clang++ -dynamiclib -o variantcaller.so -D PROGRAM -I../include -framework CoreFoundation -lobjc -liconv -Wall `pkg-config --cflags mono-2`  `pkg-config --libs-only-L mono-2` `pkg-config --variable=libdir mono-2`/libmono-2.0.a `pkg-config --libs-only-l mono-2 | sed -e "s/\-lmono-2.0 //"` embeded_libs.o ../VariantCaller.cpp
# NOW EXECUTABLE
