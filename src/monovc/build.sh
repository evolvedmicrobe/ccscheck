...
# the C++ bits
clang++ --std=c++11 -g -dynamiclib -o libVariantCaller.dylib -D PROGRAM -I/Users/nigel/git/ConsensusCore2/include -L/Users/nigel/git/pbccs/builddir/external/ConsensusCore2/build/ -lpbconsensus -I../include -framework CoreFoundation -lobjc -liconv -Wall `pkg-config --cflags mono-2`  `pkg-config --libs-only-L mono-2` `pkg-config --variable=libdir mono-2`/libmono-2.0.a `pkg-config --libs-only-l mono-2 | sed -e "s/\-lmono-2.0 //"` embeded_libs.o ../VariantCaller.cpp
# NOW EXECUTABLE
